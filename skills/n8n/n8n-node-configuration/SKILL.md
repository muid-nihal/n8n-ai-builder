...
- **[HTTP_REQUEST_REFERENCE.md](HTTP_REQUEST_REFERENCE.md)** - Complete HTTP Request node reference: all auth types, pagination modes (offset/cursor/next-URL), body types, error handling strategies, rate limiting, binary file upload/download, common integration patterns (REST, GraphQL, OAuth2), and full gotcha list. Read this when working with the HTTP Request node.
- **[GOOGLE_SHEETS_REFERENCE.md](GOOGLE_SHEETS_REFERENCE.md)** - Complete Google Sheets node reference: append vs appendOrUpdate (upsert mechanics), column mapping modes (auto vs manual), spreadsheet ID vs sheet ID formats, range specification with space-name quoting, empty read silent-stop behavior, all common errors + fixes, version regression bugs (typeVersion 4.6/4.7), and performance notes for large sheets. Read this when working with the Google Sheets node.
