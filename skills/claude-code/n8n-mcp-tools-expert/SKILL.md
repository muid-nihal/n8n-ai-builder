...
## When to Use the n8n-validator Sub-agent vs MCP Tools Directly

**Use the `n8n-validator` sub-agent** when:
- Validating a full workflow end-to-end (not just a single node)
- Doing a pre-deployment audit — agent returns a clean, structured report without flooding the conversation
- Auditing "all workflows" across the instance
- The validation output would be verbose and clutter the active build context

How to trigger: say "validate the [workflow name or ID]" — Claude delegates automatically.

**Use MCP validation tools directly** (in this conversation) when:
- Iterating on a workflow you're actively building — need to fix + revalidate in the same flow
- Checking a single node config (`validate_node`)
- Quick health check (`n8n_validate_workflow` by ID)
- You need to act on results immediately without context switching
